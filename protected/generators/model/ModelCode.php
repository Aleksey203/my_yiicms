...
class ModelCode extends CCodeModel
{
	...
	public function prepare()
	{
		...
		foreach($tables as $table)
		{
            ...
            $suffix = ((is_file($templatePath.'/backend.php'))) ? 'Base' : '';

			$this->files[]=new CCodeFile(
				Yii::getPathOfAlias($this->modelPath).'/'.$className.$suffix.'.php',
				$this->render($templatePath.'/model.php', $params)
			);
            if (is_file($templatePath.'/backend.php'))
                $this->files[]=new CCodeFile(
                    Yii::getPathOfAlias($this->modelPath).'/backend/'.$className.'.php',
                    $this->render($templatePath.'/backend.php', $params)
                );
            if (is_file($templatePath.'/frontend.php'))
                $this->files[]=new CCodeFile(
                    Yii::getPathOfAlias($this->modelPath).'/frontend/'.$className.'.php',
                    $this->render($templatePath.'/frontend.php', $params)
                );
		}
	}
